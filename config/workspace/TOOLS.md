...
**Scale**
- **56,579** Australian Instagram creators (the biggest in Australia 🇦🇺)
...
1. **Semantic search Qdrant** → candidates by natural language + vector similarity
2. **Filter payload** → follower range, gender, age_group, is_business
3. **Cross-check Hiker** → fill gaps, validate
4. **Verify live** via `creator_profile` → confirm still active, check follower drift
5. **Web search** → brand safety, recent news, any controversies
